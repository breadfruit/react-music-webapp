...
export default class Lyric {
        ...
        for(let i =0; i< lines.length; i++) {
            ...
            if(result) {
                ...
                if(txt) {
                    ...
                    this.lines.push({
                        time: result[i] * 60 * 1000 + result[2] * 1000 + (result[3] || 0) * 10,
                        txt
                    })
                }
            }
    ...
    _play(offset = 0, isSeek = false) {
        ...
        this._callHander(this.curLineIndex - 1)
        this.offset = offset;
    ...
    _playRest(isSeek = false) {
        ...
        let delay;
        if(isSeek) {
            delay = line.time - (+new Date() - this.startStamp) ? this.lines[this.curLineIndex - 1].time : 0;
        ...
        this.timer = setTimeout(() => {
            this._callHander(this.curLineIndex++) ;
